...
describe('Microtasks', function () {
    if (!global.Promise)
        return;
    function scheduleFn(task) { Promise.resolve().then(task.invoke); }
    it('should execute microtasks enqueued in the root zone', function (done) {
        var log = [];
        Zone.current.scheduleMicroTask('test', function () { return log.push(1); }, null, scheduleFn);
        Zone.current.scheduleMicroTask('test', function () { return log.push(2); }, null, scheduleFn);
        Zone.current.scheduleMicroTask('test', function () { return log.push(3); }, null, scheduleFn);
        setTimeout(function () {
    ...
    it('should correctly scheduleMacroTask microtasks vs macrotasks', function (done) {
        var log = ['+root'];
        Zone.current.scheduleMicroTask('test', function () { return log.push('root.mit'); }, null, scheduleFn);
        setTimeout(function () {
            log.push('+mat1');
            Zone.current.scheduleMicroTask('test', function () { return log.push('mat1.mit'); }, null, scheduleFn);
            log.push('-mat1');
        }, 10);
        setTimeout(function () {
            log.push('mat2');
...
//# sourceMappingURL=microtasks.spec.js.map
...
describe('XMLHttpRequest', function () {
    ...
    it('should intercept XHRs and treat them as MacroTasks', function (done) {
        ...
        testZoneWithWtf.run(function () {
            ...
            req.onload = function () {
                ...
                expect(wtfMock.log[wtfMock.log.length - 5]).toMatch(/\> Zone\:invokeTask.*addEventListener\:readystatechange/);
                expect(wtfMock.log[wtfMock.log.length - 4]).toEqual('> Zone:invokeTask:XMLHttpRequest.send("<root>::WTF::TestZone")');
                expect(wtfMock.log[wtfMock.log.length - 3]).toEqual('< Zone:invokeTask:XMLHttpRequest.send');
                expect(wtfMock.log[wtfMock.log.length - 2]).toMatch(/\< Zone\:invokeTask.*addEventListener\:readystatechange/);
                done();
            };
            req.open('get', '/', true);
            req.send();
    ...
    describe('onprogress', test_util_1.ifEnvSupports(supportsOnProgress, function () {
...
//# sourceMappingURL=XMLHttpRequest.spec.js.map
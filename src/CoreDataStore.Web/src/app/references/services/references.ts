...
export class ReferencesService {
  ...
  getBoroughs() {
    return this.http.get(`${AppSettings.ApiEndpoint}Reference/borough`).map((res: Response) => res.json());
  };

  getObjectTypes() {
    return this.http.get(`${AppSettings.ApiEndpoint}Reference/objectType`).map((res: Response) => res.json());
  };
...
//         params.set('ObjectType', objectType);
//       }
//       return this.http.get(`${AppSettings.ApiEndpoint}LPCReport/10/${pageNo}`, { search: params })
//         // .map((res: Response) => {
//         //   // console.log(0);
//         //   if (!this.properties) {
//         //     this.properties = {};
